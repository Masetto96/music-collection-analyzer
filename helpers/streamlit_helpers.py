...
def filter_by_arousal_and_valence(audio_analysis, max_valence:float, min_valence:float, max_arousal:float, min_arousal:float):
    idxs = (audio_analysis["arousal_valence"].apply(lambda x : x[0]).between(min_arousal, max_arousal)) & (audio_analysis["arousal_valence"].apply(lambda x : x[1]).between(min_valence, max_valence))
...
def filter_by_danceability(audio_analysis, danceability_upper:float, danceability_lower:float):
    return audio_analysis[(audio_analysis['danceability'] >= danceability_lower) & (audio_analysis['danceability'] <= danceability_upper)].index
...
def filter_by_key_scale(audio_analysis, key:str, scale:str):
    ...
    contains_words_mask = audio_analysis['keyscale_edma'].apply(lambda x: _contains_all_words(x, [key, scale]))
...
def plot_parent_genre_distribution(discogs_df):
    ...
    # Plot the bar chart
    fig, ax = plt.subplots()
    ax.bar(parent_genre_counts.index, parent_genre_counts.values, color='skyblue', edgecolor='black')
