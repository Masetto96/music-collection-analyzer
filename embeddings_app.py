...
st.subheader(
    " 🎛️ This app uses musiccnn and discogs-effnet embeddings to compute music similarity across tracks 🎚️ "
)

similarity_matrix_discogs = cosine_similarity(np.array(discogs))
similarity_matrix_musiccnn = cosine_similarity(np.array(musiccnn))

st.write("## 🔘 Track Selection ")
selected_track = st.select_slider(
    "The track you select will be used to compute similarity:", idxs_path, label_visibility="visible"
)
...
num_tracks = st.number_input(
    "How many tracks do you want to consider?", min_value=1, step=1, value=10, max_value=10
)
...
if st.button("# 🏃 RUN"):
    ...
    discogs_idxs = u.get_most_similar_embeddings(
        similarity_matrix=similarity_matrix_discogs, idx=track_idx, k=num_tracks
    )
    musiccnn_idxs = u.get_most_similar_embeddings(
        similarity_matrix=similarity_matrix_musiccnn, idx=track_idx, k=num_tracks
    )
